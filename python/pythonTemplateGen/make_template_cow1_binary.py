#!/usr/bin/env python3
"""
make_template_cow1.py
=====================

Generate the active cow sparse edge template for the Sobel matched-filter design.

Default flow, run from the project root:

    python python/pythonTemplateGen/make_template_cow1.py

Outputs:

    rtl/object_detection/templates/cow_edge_template_64.vh
    objects/cow_64_64tap/cow_64tap_debug_overlay.png
    objects/cow_64_64tap/cow_64tap_template_meta.json
    objects/cow_64_64tap/cow_dbg_01_gray_64.png
    objects/cow_64_64tap/cow_dbg_02_sobel_l1_mag_64.png
    objects/cow_64_64tap/cow_dbg_03_binary_edge_mask_64.png

Candidate edges mimic the RTL binary Sobel path:

    mag_l1 = abs(gx) + abs(gy)
    edge   = (mag_l1 >= EDGE_THRESH_L1) ? 8'hFF : 8'h00

The generated .vh is intended for score_tree.v / sparse_template_matcher.v and
uses this tap format:

    { row[5:0], col[5:0], signed weight[7:0] }

The design parameters that must match this template are:
...
TEMPLATE_SIZE = 64
NUM_TAPS = 64
NEGATIVE_TAPS = round(NUM_TAPS * 0.19)  # 12 guard taps, like the donut setup.
POSITIVE_TAPS = NUM_TAPS - NEGATIVE_TAPS
ROW_W = math.ceil(math.log2(TEMPLATE_SIZE))  # 6 for 64x64
COL_W = math.ceil(math.log2(TEMPLATE_SIZE))  # 6 for 64x64
WEIGHT_W = 8
POSITIVE_WEIGHT = 7
NEGATIVE_WEIGHT = -2
...
def resolve_project_root(user_root: str | None) -> Path:
    ...
    candidates.extend([Path.cwd(), Path(__file__).resolve().parents[2], Path(__file__).resolve().parent])
...
def foreground_crop(img: np.ndarray, pad_frac: float = 0.12) -> np.ndarray:
    """Crop around the cow using simple foreground segmentation."""
    h, w = img.shape[:2]
...
def sobel_magnitude(gray: np.ndarray) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    """3x3 Sobel with raw |Gx|+|Gy| magnitude, matching RTL mag_l1.

    Do not clip to 8 bits here. The hardware binary edge path thresholds the
    raw L1 magnitude first, then emits either 8'hFF or 8'h00.
    """
    gx = cv2.Sobel(gray, cv2.CV_16S, 1, 0, ksize=3, borderType=cv2.BORDER_REPLICATE)
    gy = cv2.Sobel(gray, cv2.CV_16S, 0, 1, ksize=3, borderType=cv2.BORDER_REPLICATE)
    mag_l1 = np.abs(gx).astype(np.int32) + np.abs(gy).astype(np.int32)
    return gx, gy, mag_l1


def sobel_l1_debug_u8(mag_l1: np.ndarray) -> np.ndarray:
    """Scale raw Sobel L1 magnitude to 8-bit for debug images only."""
    # Raw 3x3 Sobel L1 is normally 0..2040, so >>3 maps it to 0..255.
    return np.clip((mag_l1.astype(np.int32) + 4) >> 3, 0, 255).astype(np.uint8)


def make_edge_mask(mag_l1: np.ndarray, threshold_l1: int, dilate_radius: int) -> np.ndarray:
    """Make the same binary edge decision as sobel3x3_stream.v."""
    mask = (mag_l1 >= threshold_l1).astype(np.uint8)
    if dilate_radius > 0:
...
def select_positive_taps(mag: np.ndarray, edge_mask: np.ndarray) -> List[Tap]:
    picked = spread_pick(edge_mask, mag, POSITIVE_TAPS, min_sep=3.0)
...
def select_negative_taps(
    ...
    if len(ys) > 0:
        ...
        for idx in order:
            ...
            if any((yy - py) ** 2 + (xx - px) ** 2 < 36 for py, px in chosen):
                continue
            chosen.append((yy, xx))
...
def save_overlay(gray: np.ndarray, taps: List[Tap], path: Path) -> None:
    ...
    for tap in taps:
        ...
        cv2.circle(img, (tap.col, tap.row), 2, color, thickness=-1)
    img = cv2.resize(img, (gray.shape[1] * 6, gray.shape[0] * 6), interpolation=cv2.INTER_NEAREST)
    path.parent.mkdir(parents=True, exist_ok=True)
...
def write_meta(path: Path, image_path: Path, taps: List[Tap], args: argparse.Namespace, project_root: Path | None = None) -> None:
    meta = {
        "mode": "cow_opencv_sobel_binary_l1_sparse_matcher",
        ...
        "edge_threshold_l1": args.edge_threshold,
        "edge_output_mode": "binary_0_or_255_from_raw_l1",
        "dilate": args.dilate,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=int,
        default=330,
        help="Raw Sobel L1 threshold in RTL units: edge = abs(gx)+abs(gy) >= threshold. Default matches EDGE_THRESH_L1=330.",
    )
...
def main() -> None:
    ...
    _gx, _gy, mag_l1 = sobel_magnitude(gray)
    edge_mask = make_edge_mask(mag_l1, args.edge_threshold, args.dilate)

    pos_taps = select_positive_taps(mag_l1, edge_mask)
    neg_taps = select_negative_taps(mag_l1, edge_mask, pos_taps, args.neg_gap, rng)
    ...
    save_gray(sobel_l1_debug_u8(mag_l1), docs_dir / f"cow_dbg_02_sobel_l1_mag_{TEMPLATE_SIZE}.png")
    save_gray((edge_mask * 255).astype(np.uint8), docs_dir / f"cow_dbg_03_binary_edge_mask_{TEMPLATE_SIZE}.png")
