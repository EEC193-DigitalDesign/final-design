...
matched filter right now: raw Sobel L1 edge threshold, dilation, negative guard
gap, and random seed for deterministic guard-tap placement.
...
def make_edge_mask(mag_l1: np.ndarray, threshold_l1: int, dilate_radius: int) -> np.ndarray:
    ...
    return dilate(mag_l1 >= threshold_l1, dilate_radius)
...
def write_meta(path: Path, image_path: Path, taps: List[Tap], edge_threshold_l1: int, dilate_radius: int, neg_gap: int, seed: int, project_root: Path | None = None) -> None:
    meta = {
        "mode": "donut_sobel_binary_l1_sparse_matcher_only",
        ...
        "edge_threshold_l1": edge_threshold_l1,
...
def main() -> None:
    ...
    save_gray(sobel_l1_debug_u8(mag_l1), docs_dir / f"donut_dbg_02_sobel_l1_mag_{TEMPLATE_SIZE}.png")
    save_gray((edge_mask * 255).astype(np.uint8), docs_dir / f"donut_dbg_03_binary_edge_mask_{TEMPLATE_SIZE}.png")
