...
def write_metrics(path: Path, *, expected: dict[tuple[int, int], int], observed: dict[tuple[int, int], int], mismatches: list[str]) -> None:
...
@cocotb.test()
async def sparse_template_matcher_score_test(dut):
    width = env_int("TEST_WIDTH", 128)
    height = env_int("TEST_HEIGHT", 96)
    template_size = env_int("TEMPLATE_SIZE", 64)
    num_taps = env_int("NUM_TAPS", 32)
    donut_x = env_int("DONUT_X", max(0, (width - template_size) // 2))
    donut_y = env_int("DONUT_Y", max(0, (height - template_size) // 2))
    output_dir = Path(os.environ.get("OUTPUT_DIR", "output/sparse_matcher"))
    ...
    try:
        ...
        feature = make_feature_frame(width, height, template_size, donut_x, donut_y)
        expected = reference_scores(feature, taps, template_size)

        # Stream-delay flush plus score-tree/MAC latency margin.
        flush_cycles = template_size + num_taps + 32
        ...
        write_metrics(output_dir / "compare_metrics.txt", expected=expected, observed=observed, mismatches=mismatches)
        ...
        assert not mismatches, "DUT sparse matcher did not match Python reference; see sim/output/sparse_matcher/compare_metrics.txt"
        assert best_dut_coord == best_ref_coord, "Best-match coordinate differs from reference"
    except Exception:
